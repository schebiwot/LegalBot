...
;
            ...
            try{
              ...
              var counties = JsonConvert.DeserializeObject<List<County>>(json);
                ...
                if((string) stepContext.Values["preferredLanguage"] == "English"){
                        ...
                        var promptOptions =  new PromptOptions{
                                Prompt = MessageFactory.Text("Which county do you live in(select a county) ?\n" + options),
        ...
        // our validators...
        private Task<bool>CountyStepValidatorAsync(PromptValidatorContext<int> promptValidatorContext,CancellationToken cancellationToken){
            var valid = false;
            if(promptValidatorContext.Rec)
        }
