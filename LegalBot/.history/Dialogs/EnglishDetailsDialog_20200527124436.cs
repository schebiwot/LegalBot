        ...
        private async Task<DialogTurnResult> SubCountyStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            var counties = JsonConvert.DeserializeObject<List<County>>(json);
                ...
                for(int i =0; i<counties.Count; i++){ 
                            ...
                            {
                               var sub = JsonConvert.DeserializeObject<List<SubCounty>>(counties[i].Sub);
                               Console.WriteLine("well",sub);
                               
                            //    for(int j = 0; j< counties[i].SubCountyName.Count;j++){
                            //        Console.WriteLine("hmmm",counties[i].SubCountyName[j]);
                            //    }
                            }
            ...
            if((string) stepContext.Values["preferredLanguage"] == "English"){
                ...
                var promptOptions = new PromptOptions{
                    ...
                    Prompt = MessageFactory.Text("Which sub-county are you located at?:" + sub),
                    RetryPrompt = MessageFactory.Text("Please choose an option from the list."),
