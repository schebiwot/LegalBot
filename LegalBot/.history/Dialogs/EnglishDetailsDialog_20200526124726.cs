    ...
    public class EnglishDetailsDialog :ComponentDialog
    {
        ...
        {   
            ...
            try{
               ...
               Dictionary<string, string> values = JsonConvert.DeserializeObject<Dictionary<int,string>>(json);

                // var m = counties.ToList();
                if((string) stepContext.Values["preferredLanguage"] == "English"){
                        
                            
                        var promptOptions =  new PromptOptions{
                                Prompt = MessageFactory.Text("Which county do you live in ?" + values["countyid"]),
